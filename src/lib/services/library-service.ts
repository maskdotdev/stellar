...
export class LibraryService {
	...
	async downloadPdfFromUrlAndProcessBackground(
		options: UploadPdfFromUrlOptions,
	): Promise<Document> {
		try {
			console.log(
				"Starting PDF download from URL (background processing):",
				options,
			);

			// Download the PDF and create document immediately (background processing for content)
			const document = await invoke<Document>(
			...
			console.log("PDF downloaded and document created:", document);
			return document;
		} catch (error) {
	...
	async updateDocument(
		...
		try {
			const document = await invoke<Document | null>("update_document", {
				id,
				request,
			});
	...
	getContentPreview(content: string, maxLength = 200): string {
		if (!content) return "";
	...
	async updateCategory(
		...
		try {
			const category = await invoke<Category | null>("update_category", {
				id,
				request,
			});
	...
	async getDocumentsByCategory(categoryId: string): Promise<Document[]> {
		try {
			const documents = await invoke<Document[]>("get_documents_by_category", {
				categoryId,
			});
			return documents;
		} catch (error) {
