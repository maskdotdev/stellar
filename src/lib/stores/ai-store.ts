...
// 🔥 NEW: Document reference for chat integration
...
// 🔥 NEW: Enhanced conversation interface with study context
export interface AIConversation {
  ...
  // 🔥 NEW: Study context integration
  sessionId?: string
  documentReferences: DocumentReference[]
  categoryTags: string[]
  learningObjectives?: string[]
  conversationType: 'question' | 'explanation' | 'review' | 'brainstorm' | 'general'
  studyContext: {
...
interface AIState {
  ...
  // 🔥 NEW: Smart conversation management
  ...
  updateConversationStudyContext: (
    conversationId: string,
    studyContext: Partial<AIConversation['studyContext']>
  ) => void
  linkConversationToSession: (conversationId: string, sessionId: string) => void
  findConversationsByDocument: (documentId: string) => AIConversation[]
  findRelatedConversations: (conversationId: string) => AIConversation[]
...
export const useAIStore = create<AIState>()(
  persist(
    (set, get) => ({
      ...
      // 🔥 NEW: Smart conversation management methods
      createConversationWithContext: (title, sessionId, documentReferences = [], conversationType = 'general') => {
        const conversationId = crypto.randomUUID()
        const conversation = {
          title,
          messages: [],
          ...
          sessionId,
          documentReferences,
          categoryTags: documentReferences.map(doc => doc.category_id).filter(Boolean) as string[],
          conversationType,
          studyContext: {
            relatedConversations: [],
            conceptsCovered: [],
      ...
      updateConversationStudyContext: (conversationId, studyContext) => set((state) => ({
        conversations: state.conversations.map(c => 
          c.id === conversationId 
            ? { ...c, studyContext: { ...c.studyContext, ...studyContext } }
            : c
        )
      })),
      
      linkConversationToSession: (conversationId, sessionId) => set((state) => ({
        conversations: state.conversations.map(c => 
          c.id === conversationId 
            ? { ...c, sessionId }
            : c
      ...
      findConversationsByDocument: (documentId) => {
        const state = get()
        return state.conversations.filter(c => 
          c.documentReferences.some(ref => ref.id === documentId)
        )
      },
      
      findRelatedConversations: (conversationId) => {
        const state = get()
        ...
        return state.conversations.filter(c => 
          c.id !== conversationId && 
          (c.sessionId === targetConversation.sessionId ||
           c.documentReferences.some(ref => 
             targetConversation.documentReferences.some(targetRef => targetRef.id === ref.id)
           ))
        )
      },
