...
import { Edit, FolderPlus, GraduationCap, Trash2 } from "lucide-react";
...
export function CategoryView({
  ...
  if (filteredCategories.length === 0 && showUncategorized) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        <GraduationCap className="h-12 w-12 text-muted-foreground mb-4" />
        <h3 className="text-lg font-medium mb-2">
          {categories.length === 0
            ? "Welcome to your Study Library!"
            : "No categories found"}
        </h3>
  ...
  return (
    ...
    >
      ...
      {filteredCategories.map((category, index) => {
        const displayNumber = index + (showUncategorized ? 2 : 1); // adjust numbering
