...
export function useLibrary() {
    ...
    const initializeLibrary = async () => {
      try {
        ...
        const [categoriesData, documentsData] = await Promise.all([
          ...
          currentCategory ? libraryService.getDocumentsByCategory(currentCategory) : []
        ])
      ...
      const loadCategoryDocuments = async () => {
        try {
          ...
          if (currentCategory === "uncategorized") {
            ...
            documentsData = await libraryService.getDocumentsByCategory(currentCategory)
          }
  ...
  const filteredCategories = currentCategory === null ? categories.filter(
    (category) =>
      category.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      (category.description && category.description.toLowerCase().includes(searchQuery.toLowerCase()))
  ) : []

  const filteredDocuments = currentCategory !== null ? documents.filter(
    (item) =>
  ...
  const handleUpdateCategory = async () => {
    ...
    try {
      const updatedCategory = await libraryService.updateCategory(editingCategory.id, categoryForm)
      if (updatedCategory) {
  ...
  const handleDeleteCategory = async (categoryId: string) => {
    try {
      ...
      if (success) {
        ...
        toast({
          title: "Success",
          description: "Category deleted successfully. Documents have been moved to uncategorized.",
        })
      }
    } catch (error) {
  ...
  const handleSuggestedCategorySelect = (suggested: typeof import('./library-constants').suggestedCategories[0]) => {
    setCategoryForm({
      name: suggested.name,
